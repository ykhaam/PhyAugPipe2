...
기본 category 목록은 `configs/action_categories.txt`를 사용합니다.
...
`S_f(k) = mean(VideoPhy2 score on top-nc representatives)`
...
- `H_f(k)`: Stage C 이후 category별 보유 샘플 수
- `N`: `--N` (또는 `--budget`)으로 주는 총 예산
- `τ`: `--tau` (기본 1.0)
...
- 대표 샘플(top-nc = `--representative_topk`)은 각 category에서 `action_match_score` 내림차순 상위 샘플로 고릅니다.
- 기본 모드(`--videophy2_mode auto`)에서는 위 대표 샘플을 VideoPhy2로 자동 평가합니다.
  ...
  - bridge는 VideoPhy2 출력에 `semantics_adherence_score`와 `physics_commonsense_score`가 둘 다 있으면 두 점수 평균을 최종 category score 계산용 점수로 사용합니다.
