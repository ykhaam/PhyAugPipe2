...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="videophy2_bridge_") as tmp_dir:
        ...
        sem_col = _pick_column(scored, ["semantics_adherence_score", "semantic_score", "text_video_alignment_score"])
        phy_col = _pick_column(scored, ["physics_commonsense_score", "physics_score", "commonsense_score"])
        if sem_col and phy_col:
            sem_vals = pd.to_numeric(scored[sem_col], errors="coerce")
            phy_vals = pd.to_numeric(scored[phy_col], errors="coerce")
            scored[args.score_field] = (sem_vals + phy_vals) / 2.0
            score_col = args.score_field
        else:
